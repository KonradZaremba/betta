...
namespace Betta.Quickstart_Next
{
    /// <summary>
    ...
    /// </summary>
    [GrasshopperCollection("Quickstart", "Demo_Next")]
    public interface IQuickstart : IBettaCollection
    {
        [GrasshopperMethod("Cube_From_Next", "x³ — cube of a number")]
        double Cube([GrasshopperParameter("Value", DefaultValue = 2.0)] double x);

        [GrasshopperMethod()]
        double Cube_Simple(double x);
        ...
        [GrasshopperMethod("Stats_Next", "Sum and average of a list")]
        (double Sum, double Average) Test2(
    [GrasshopperParameter("Numbers", "N", "The numbers to summarize")] List<double> numbers);

        [GrasshopperMethod()]
        (double Sum, double Average) Test3(List<double> numbers);
    ...
    public class Quickstart : IQuickstart
    {
        ...
        public double Cube_Simple(double x) => x * x * x;


        public (double Sum, double Average) Stats(List<double> numbers)
        {
            return CoreFunction(numbers);
        }
        [GrasshopperMethod("Test")]
        public (double Sum, double Average) Test(List<double> numbers)
        {
            return CoreFunction(numbers);
        }

        public (double Sum, double Average) Test2([GrasshopperParameter("Numbers", "N", "The numbers to summarize")] List<double> numbers)
        {
            return CoreFunction(numbers);
        }

        public (double Sum, double Average) Test3(List<double> numbers)
        {
            return CoreFunction(numbers);
        }

        private (double Sum, double Average) CoreFunction(List<double> numbers)
        {
